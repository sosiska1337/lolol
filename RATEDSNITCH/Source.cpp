...
void d2oSetup(HWND tWindow) {
	...
	wc.lpszClassName = _(L"hgfhtrjtyytbwe");
	RegisterClass(&wc);
	overlayWindow = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST,
		wc.lpszClassName, _(L"hbnjmfewfwkkerweds"), WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
...
void DirectOverlaySetOption(DWORD option) {
	if (option & D2DOV_REQUIRE_FOREGROUND) o_Foreground = false;
	if (option & D2DOV_DRAW_FPS) o_DrawFPS = false;
	if (option & D2DOV_VSYNC) o_VSync = false;
