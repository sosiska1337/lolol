...
// Wrapper:
///////////////////////////////////////////////
bool CHTTPSession::Send(LPCWSTR wcDomain, LPCWSTR wcDirectory, std::string wParameters)
{
	...
	// Send a request.
	if (SendRequest(wParameters) == false)
		return false;
...
// Wrapper:
///////////////////////////////////////////////
int CHTTPSession::Request(int iHTTP_Method, int iHTTP_Protocol, std::string sTarget, std::string& sRecvData, std::string sParameters, int iEncyption)
{
	...
	if (Send(wsDomain.c_str(), wsDocument.c_str(), sParameters) == false)
		return 4;
...
bool CHTTPSession::SendRequest(std::string sParameters)
{
	if (!hRequest)
		return false;
	...
	{
		std::string sPOSTDATA = sParameters;
