...
auto FOverlay::DrawBox(float x, float y, float width, float height, float thickness, float r, float g, float b, float a, bool filled)-> void
{
	brush->SetColor(D2D1::ColorF(r, g, b, a));
	if (filled)  tar->FillRectangle(D2D1::RectF(x, y, x + width, y + height), brush);
	else tar->DrawRectangle(D2D1::RectF(x, y, x + width, y + height), brush, thickness);
}
...
auto FOverlay::draw_text_red(int x, int y, const char* str, ...)-> void {
	...
	tar->DrawText(b, len, format, D2D1::RectF(x, y, 1920, 1080), red_brush, D2D1_DRAW_TEXT_OPTIONS_NONE, DWRITE_MEASURING_MODE_NATURAL);
}
...
auto FOverlay::draw_text_green(int x, int y, const char* str, ...)-> void {
	...
	tar->DrawText(b, len, format, D2D1::RectF(x, y, 1920, 1080), green_brush, D2D1_DRAW_TEXT_OPTIONS_NONE, DWRITE_MEASURING_MODE_NATURAL);
}
